...
import (
	...
	broccoliconfig "github.com/elvisNg/broccoli/config"
	"github.com/elvisNg/broccoli/utils"
)

var broccoliCfg *broccoliconfig.AppConf

const OBS_ROOTPATH_KEY = "obs_root"

func InitBroccoliCfg(cfg *broccoliconfig.AppConf) {
	broccoliCfg = cfg
}
...
func (conf *config) formatUrls(bucketName, objectKey string, params map[string]string, escape bool) (requestUrl string, canonicalizedUrl string) {
	rootPath := broccoliCfg.EBus.PathMap[OBS_ROOTPATH_KEY]
	if !utils.IsEmptyString(broccoliCfg.Obs.Endpoint) {
		rootPath = "/"
	}
	urlHolder := conf.urlHolder

	if bucketName == "" {
		requestUrl = fmt.Sprintf("%s://%s:%d%s", urlHolder.scheme, urlHolder.host, urlHolder.port, rootPath)
		canonicalizedUrl = "/"
	} else {
		requestUrl = fmt.Sprintf("%s://%s:%d%s%s", urlHolder.scheme, urlHolder.host, urlHolder.port, rootPath, bucketName)
		canonicalizedUrl = "/" + bucketName
	}
