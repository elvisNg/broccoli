...
import (
	...
	client "github.com/elvisNg/broccoli/obs"
	"github.com/elvisNg/broccoli/obs/obs"
)
...
const OBS_ROOTPATH_KEY = "obs_root"
...
func init() {
	...
	conf.EBus = config.EBus{
		Hosts:     []string{"http://111.230.222.29:8080"},
		PaasId:    "dzqzportal",
		PaasToken: "nDJA6xjwY2UyacR0fq0tNG3OMo63VwGC",
		PathMap:   map[string]string{OBS_ROOTPATH_KEY: "/ebus/obs/"},
	}
	conf.Obs = config.Obs{
		AK:         "OM65GZAEBJQAX9EFZVAX",
		SK:         "d4VGQ3gqxh8uWtbN18wKyUIhe1cbF16PAW2l6MAJ",
		BucketName: "obs-zf-dzqz-bak",
		Location:   "",
	}
	endpoint = conf.EBus.Hosts[0]
	ak = conf.Obs.AK
	sk = conf.Obs.SK
	bucketName = conf.Obs.BucketName
	objectKey = "object-test"

	client.InitDefault(conf)
	client.ReloadDefault(conf)
	// client.ReleaseDefault()
}

func upload() string {
	input := strings.NewReader("Hello OBS")
	key, err := client.Put(context.Background(), input, "")
	if err != nil {
		fmt.Println(err)
		return ""
	}
...
func download(key string) {
	...
	output, err := client.Get(context.Background(), key)
	if err == nil {
		defer output.Close()
		p := make([]byte, 1024)
		var readErr error
		var readCount int
		for {
			readCount, readErr = output.Read(p)
			if readCount > 0 {
				fmt.Printf("%s\n", p[:readCount])
			}
			if readErr != nil {
...
func checkExist(key string) {
	...
	output, err := client.GetMeta(context.Background(), key)
	if err == nil {
		fmt.Printf("======%+v======\n", output)
	} else {
		fmt.Printf("======%+v======\n", err)
...
func main() {
	//---- init log ----
	obs.InitLog("./OBS-SDK.log", 1024*1024*100, 5, obs.LEVEL_WARN, false)
...
func copyPart() {
	...
	sourceBucket := "resources-bucket"
	sourceKey := "resources-key"
	input := &obs.GetObjectMetadataInput{}
